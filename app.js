...
let userInput = document.getElementById("txt-search");
let searchedList = [];

//main function to search for videos
function searchVideo () {
  ...
  //if user does not enter a keyword
  if (search.value == "") {
  ...
  // displaying videos that match keyword
  videosList.map((item,index)=>{

    if (item.description.includes(search.value)) {

      let printList = generateRowHTML(item);
  ...
  // if no videos match keyword
  if (flag == false) {

    let listError = (`No videos found with the keyword ${search.value}`);
    document.getElementById("results").innerHTML = listError;
...
function showAll () {
  ...
  videosList.map((item,index)=>{
      ...
      let allVids = generateRowHTML(item);
      document.getElementById("results").innerHTML += allVids;
